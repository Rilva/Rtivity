...
updateDates <- function(){
  ...
  n <- getFileNr()
  Date1 <- as.Date(dates$startDateTime[n], format = "%Y-%m-%d %H:%M:%S")
  Date2 <- as.Date(dates$stopDateTime[n], format = "%Y-%m-%d %H:%M:%S")
  Time1 <- as.POSIXct(dates$startDateTime[n], format = "%H:%M:%S")
  Time2 <- as.POSIXct(dates$stopDateTime[n], format = "%H:%M:%S")
  
  if(strftime(dates$startDateTime[n],format = "%H:%M:%S") == "00:00:00"){
   Date1 <- Date1 + 1  
  }
  if(strftime(dates$stopDateTime[n],format = "%H:%M:%S") == "00:00:00"){
    Date2 <- Date2 + 1  
  }
...
observeEvent(input$files,{
  ...
  #Get directory of the temporary files
  validate(
  ...
  #Rename the temporary files for the actual file names
  for(i in 1:nrow(input$files)){
    file.rename(input$files$datapath[i], paste(path,input$files$name[i],sep="/"))
  }
  
  #### If the files were imported, reset variablles
  ##### RESET  variables 
  ...
  withProgress(message = 'Importing files', value = 0, {
    ...
    #Import only the text files that were selected
    ...
    "
    Check for each file if the data:
    1) is in table format
    2) is in 42 column format (44 column being the column 2, 3 and 4 one column (day, month year))
    3) has missing data
    "
    ...
    for (i in 1:length(files)){
      ...
      #Check if data is in 42 column format
      if (ncol(data)==44){
        if (is.null(import)){
    ...
    validate(
      need(length(MissingDataFiles)==0, message = ""),
      need(length(ErrorColumnNumberFile)==0, message = ""),
    ...
    ZT(zeitgeberTime) #Zeitgeber time variable (light onset time)
...
observeEvent(input$zeitgeberTime,{
  ...
  # If there are any files, update the zeitgeber time variable aaccording to users change
  
  req(length(ImportedFiles())>0)
  n <- getFileNr() #Get file oreder
...
observeEvent(changeDate(),{
  ...
  if (timeDiff <1){
    updateRadioButtons(session,'SleepBoxTime','Box time', c("Minutes" = "Min", "Hours" = "Hour"), selected = "Hour", inline = TRUE)
    updateRadioButtons(session,'ActivityBoxTime','Box time', c("Minutes" = "Min", "Hours" = "Hour"), selected = "Hour", inline = TRUE)
  }
  else{
    updateRadioButtons(session,'SleepBoxTime','Box time', c("Hours" = "Hour", "Days" = "Day"), selected = "Day", inline = TRUE)
...
observeEvent(Conditions$df,{
  ...
  output$saveMetadata <- downloadHandler(
    ...
    content = function(file){
      ...
      addDataFrame(settingsTable(), sheet=sheet, startColumn=1, row.names=FALSE)
      
      #Colors sheet
      if(nrow(graphsAestethics$df)>0 & nrow(damData$dt)>0){
        sheet <- createSheet(wb, "Colors")
        addDataFrame(graphsAestethics$df, sheet=sheet, startColumn=1, row.names=FALSE)
      }
