...
updateVolumes <- function(){
  vol <- c("Home" = '~' ,"curr_dir" = getwd(), "C"="C:/")
  
  for (i in 4:length(LETTERS)){
    if (dir.exists(paste0(LETTERS[i],":/"))) {
      vol <- c(vol,"A" = paste0(LETTERS[i],":/"))
      names(vol)[length(vol)]<-LETTERS[i]
    }}
...
#Delete row from table data and from metadata
...
observe({
  ...
  output$saveMetadata <- downloadHandler(
    ...
    addDataFrame(Conditions$df[1:6], sheet=sheet, startColumn=1, row.names=FALSE)
...
observeEvent(input$startanalysis,{
  ...
  withProgress(message = 'Analyzing data (1/2)', value = 0, {
    #get DAM data
    data <- link_dam_metadata(Conditions$df,Directory()) #linking
    damData$dt <- load_dam(data, FUN = sleep_dam_annotation, min_time_immobile = 60*input$sleepTime) #load dam data
    ...
    if ((max(damData$dt[,'t'])-min(damData$dt[,'t']))/60 < 180){
      ...
      updateSliderInput(session,"sleepTime",value = 10, max = 180)
    }
...
observe({
  ...
  withProgress(message = 'Analyzing data(2/2)', value = 0, {
    ...
    updateSliderInput(session,"movingAverage", min = round(mean(damData$dt[,timeDiff],na.rm=TRUE)/60), step = max(damData$dt[,timeDiff],na.rm=TRUE)/60)
    updateSliderInput(session,"boutWindow", min = round(mean(damData$dt[,timeDiff],na.rm=TRUE)/60), step = max(damData$dt[,timeDiff],na.rm=TRUE)/60)
  ...
  if ("periodT" %in% colnames(damData$dt)){
    update_periodT()
  }
...
observeEvent(input$remove,{
  ...
  for (i in 1:length(finalTimes)){
    ...
    if (input$removeWindow == "mins"){
      dt_curated <- curate_dead_animals(damData$dt[t %between% c(finalTimes[i]-mins(input$deadTime), finalTimes[i])], prop_immobile = 0, time_window= mins(input$deadTime))}
    else{
      dt_curated <- curate_dead_animals(damData$dt[t %between% c(finalTimes[i]-hours(input$deadTime), finalTimes[i])], prop_immobile = 0, time_window= hours(input$deadTime))
    }
  ...
  if (length(indexesToDelete>0)){
    dead <- link_dam_metadata(Conditions$df[indexesToDelete,],Directory()) #linking
    Dead_animal(load_dam(dead, FUN = sleep_dam_annotation, min_time_immobile = 60*input$sleepTime)) #load dam data of dead animals
    
    live <- Conditions$df[-indexesToDelete,]
    
    live <- link_dam_metadata(live,Directory()) #linking
    Live_animal(load_dam(live, FUN = sleep_dam_annotation, min_time_immobile = 60*input$sleepTime)) #load dam data of live animals
    
    shinyjs::enable("Death_graphs")
    shinyjs::enable('deleteInactivity')
  }
  
  shinyjs::enable('deleteLarvae')
  shinyjs::enable('deleteInactivity')
...
observeEvent(input$deleteLarvae,{
...
output$checkChannels <- renderPlot({
  ...
  validate(
    need (!is.null(damData$dt),"No data selected")
  )
